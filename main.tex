...
\begin{enumerate}
	...
	\item[] - \textcolor{orange}{Blockchain (technology)}: A blockchain is a growing list of records, called blocks, that are linked using cryptography. Each block contains a cryptographic hash of the previous block, a timestamp, and transaction data  
	\item[] - \textcolor{orange}{Electronic currency} VS. \textcolor{orange}{Digital currency}
	\item Secure Multiparty Computation (MPC or SMC)
	\item[] - Secure multi-party computation is a subfield of cryptography with the goal of creating methods for parties to jointly compute a function over their inputs while keeping those inputs private. 
\end{enumerate}
...
Two millionaires, Alice and Bob, who are interested in knowing which of them is richer \underline{without revealing their actual wealth}.
\begin{figure}
	\includegraphics[width=3.1 in, height=1.9in]{myfigs/SMC.jpg}
\end{figure}
\colorbox{blue}{A joke}: It's not an example for Yao's Millionaires' Problem and it should an example of Yao's Billionaires' Problem...
...
- Online gambling \newline
- Online lottery \newline
- Other activities involving mutual trust
\end{frame}
...
Alice drafts a transaction alerting that she transfer v money to Bob. And 
\end{alertblock}

\begin{block}{}
1. Alice references an existing transaction in which she is the receiver of a certain amount of money. \textcolor{red}{$\Longrightarrow$ //There exists some \underline{unspent} money.}

2. Alice signs the transaction with her private key. \textcolor{red}{$\Longrightarrow$ //Alice: I am indeed the owner of the money.}
\end{block}
